...
import { GET_USERS_QUERY, GET_USER_QUERY } from "@/lib/graphql/queries/user";
...
export default function EmployeeEditFormWithAPI({ employee, onSuccess, onCancel }: EmployeeEditFormProps) {
  const [formData, setFormData] = useState({
    firstName: employee.firstName || "",
    lastName: employee.lastName || "",
    email: employee.email || "",
    password: "",
    role: (employee.role?.toLowerCase?.() || "employee") as UserRole,
  });
  ...
  // If the employee prop changes (route change), sync form state
  useEffect(() => {
    setFormData({
      firstName: employee.firstName || "",
      lastName: employee.lastName || "",
  ...
  const [updateUser, { loading: mutationLoading }] = useMutation(UPDATE_USER_MUTATION, {
    ...
    refetchQueries: [
      ...
      { query: GET_USER_QUERY, variables: { id: employee.id } },
    ],
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateUser({ variables: { id: employee.id, input } });
    } catch (err) {}
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex justify-between pt-4">
        ...
        <SubmitButton isLoading={mutationLoading}>Update Employee</SubmitButton>
      </div>
    </form>
  );
}
